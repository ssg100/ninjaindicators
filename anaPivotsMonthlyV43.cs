...
public enum anaPivotStylesPM43 {Floor, Wide, Fibonacci, Jackson, Camarilla, Woodie}
public enum anaSessionTypePM43 {DailyBars, ETH, RTH, Auto, UserDefined}
public enum anaSessionCountPM43 {First, Second, Third}
public enum anaCalcModePM43 {DailyClose, IntradayClose} 
public enum anaPlotAlignPM43 {Left, Right, LeftAll, RightAll, DoNotPlot}
...
namespace NinjaTrader.Indicator
{
	...
	public class anaPivotsMonthlyV43: Indicator
	{
		#region Variables
		...
		private anaPlotAlignPM43	plotLabels				= anaPlotAlignPM43.RightAll;
		private anaPivotStylesPM43 	pivotFormula			= anaPivotStylesPM43.Floor;
		private anaSessionTypePM43	pivotSession			= anaSessionTypePM43.Auto;
		private anaSessionTypePM43	selectedSession			= anaSessionTypePM43.ETH;
		private anaSessionCountPM43 activeSession			= anaSessionCountPM43.Second;
		private anaCalcModePM43		calcMode				= anaCalcModePM43.DailyClose;
		...
		private SolidBrush			pivotBrush				= new SolidBrush(Color.Peru);			
		private SolidBrush			centralBrush			= new SolidBrush(Color.Maroon);			
		...
		private	string				errorData3				= "The prior close detected by the Monthly Pivots indicator is not within the range of the selected RTH session.";
		private	string				errorData4				= "Insufficient historical data for the Monthly Pivots indicator. Please increase chart look back period.";
		...
		private int					opacityPR				= 3;
		private int					opacityJZ				= 3;
		...
		private Color 				pivotColor				= Color.SandyBrown;
		private Color 				resistanceColor 		= Color.PeachPuff;
		private Color 				supportColor	 		= Color.PeachPuff;
		private Color				midpivotColor			= Color.PeachPuff;
		private Color				highColor				= Color.Lime;
		private Color				lowColor				= Color.OrangeRed;	
		private Color				closeColor				= Color.Yellow;
		private Color				centralPivotColor		= Color.SandyBrown;
		private Color				directionalPivotColor	= Color.SandyBrown;
		private Color				pivotRangeColor			= Color.Peru;
		private Color				centralRangeColor		= Color.Firebrick;
		...
		{
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-PP"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-R1"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-S1"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-R2"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-S2"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-R3"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-S3"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-R4"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-S4"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-R5"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-S5"));
			
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-RMid"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-SMid"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-R12Mid"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-S12Mid"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-R23Mid"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-S23Mid"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-R34Mid"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-S34Mid"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-R45Mid"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-S45Mid"));
	
			Add(new Plot(new Pen(Color.Gray,2), PlotStyle.Line,"PM-H"));
			Add(new Plot(new Pen(Color.Gray,2), PlotStyle.Line,"PM-L"));
			Add(new Plot(new Pen(Color.Gray,2), PlotStyle.Line,"PM-C"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-CP"));
			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"M-DP"));

			Add(new Plot(new Pen(Color.Gray,1), PlotStyle.Line,"PM-VWAP"));
		...
		protected override void OnBarUpdate()
		{
			...
			if (CurrentBar == 0)
			{
				if (selectedSession == anaSessionTypePM43.UserDefined)
				{
				...
				else
				{
					...
					sessionCount = 1;
					if (selectedSession == anaSessionTypePM43.DailyBars && currentDate.Day == 1)
						countDown = 0;
				}
				return;
			}
			if(FirstTickOfBar && Bars.BarsType.IsIntraday && (selectedSession == anaSessionTypePM43.DailyBars || (selectedSession != anaSessionTypePM43.UserDefined && calcMode == anaCalcModePM43.DailyClose)))
			{	
				...
				if (existsHistDailyData) 
				{
					...
					if(selectedSession == anaSessionTypePM43.DailyBars)
					{
						high = dailyBar.High;
						low = dailyBar.Low;
					}
			...
			else if (selectedSession != anaSessionTypePM43.DailyBars)
			{
				high = High[0];
				low = Low[0];
			}
			...
			if (isEndOfMonthHoliday && selectedSession != anaSessionTypePM43.RTH)
				lastBarTimeStamp3 = GetLastBarSessionDate(Time[0].AddDays(3), Bars, 0, pivotRangeType3);
			if (lastBarTimeStamp3 != currentMonth || selectedSession == anaSessionTypePM43.UserDefined)
			{
				if (countDown == 0)
				{
				...
				if ((countDown < 0) || selectedSession == anaSessionTypePM43.UserDefined)
				{
					range = (currentHigh - currentLow);						
					if (PivotFormula == anaPivotStylesPM43.Fibonacci)
					{
					...
					else if (PivotFormula == anaPivotStylesPM43.Jackson)
					{
						pp				= (currentHigh + currentLow + currentClose)/ 3;
						r1				= pp + 0.500*range;
						s1				= pp - 0.500*range;
					...
					else if (PivotFormula == anaPivotStylesPM43.Camarilla)
					{
						pp				= (currentHigh + currentLow + currentClose)/ 3;
					...
					{
						if (PivotFormula == anaPivotStylesPM43.Floor || PivotFormula == anaPivotStylesPM43.Wide)
							pp			= (currentHigh + currentLow + currentClose) / 3;
						else if (PivotFormula == anaPivotStylesPM43.Woodie)
						...
						r2				= pp + range;
						s2				= pp - range;
						if (PivotFormula == anaPivotStylesPM43.Floor || PivotFormula == anaPivotStylesPM43.Woodie)
						{
							r3				= r1 + range;
							s3				= s1 - range;
							r4				= r3 + (pp - currentLow);
						...
						else if (PivotFormula == anaPivotStylesPM43.Wide)
						{
							r3				= r2 + range;
							s3				= s2 - range;
							r4				= r3 + range;
					...
					if (showMidpivots && PivotFormula != anaPivotStylesPM43.Camarilla && 
						PivotFormula != anaPivotStylesPM43.Fibonacci && PivotFormula != anaPivotStylesPM43.Jackson)
					{
				...
				if (selectedSession != anaSessionTypePM43.UserDefined)
				{
					if (Bars.BarsType.IsIntraday && (!existsHistDailyData || (existsHistDailyData && selectedSession != anaSessionTypePM43.DailyBars))) 
					{
						if (numberOfSessions == 1 || selectedSession == anaSessionTypePM43.ETH || (selectedSession == anaSessionTypePM43.RTH && activeSession == anaSessionCountPM43.First))
						{
							if (!existsHistDailyData)	
								currentClose	= close;
							currentHigh			= high;
			...
			else if (selectedSession != anaSessionTypePM43.UserDefined && (lastBarTimeStamp1 != currentDate || Bars.Period.Id == PeriodType.Day))
			{
				firstDayOfPeriod = false;
				sessionCount = 1;
				priorDate = currentDate;
				currentDate = lastBarTimeStamp1;
				if (Bars.BarsType.IsIntraday && existsHistDailyData && pivotSession == anaSessionTypePM43.DailyBars) 
				{
				...
				{
					if (numberOfSessions == 1 || pivotSession == anaSessionTypePM43.ETH || (pivotSession == anaSessionTypePM43.RTH && activeSession == anaSessionCountPM43.First))
					{
						if (!existsHistDailyData)	
							currentClose		= close;
						currentHigh 		= Math.Max(currentHigh, high);
			...
			else if (selectedSession != anaSessionTypePM43.UserDefined && Bars.BarsType.IsIntraday && Bars.FirstBarOfSession)
			{
				if (FirstTickOfBar)
				{
				...
				if (existsHistDailyData && selectedSession == anaSessionTypePM43.DailyBars) 
				{
					if (FirstTickOfBar)
					{
						volSum			= currentVolSum;
				...
				else if (numberOfSessions == 1 || selectedSession == anaSessionTypePM43.ETH)
				{
					if (!existsHistDailyData)	
						currentClose		= close;
					currentHigh			= Math.Max(currentHigh, high);
				...
				else if ( firstDayOfPeriod && ((sessionCount == 1 && activeSession == anaSessionCountPM43.First) || (sessionCount == 2 
					&& activeSession == anaSessionCountPM43.Second) || (sessionCount == 3 && activeSession == anaSessionCountPM43.Third)))
				{
					if (!existsHistDailyData)	
						currentClose	= close;
					currentHigh			= high;
				...
				else if ((sessionCount == 1 && activeSession == anaSessionCountPM43.First)||(sessionCount == 2 && 
					activeSession == anaSessionCountPM43.Second) || (sessionCount == 3 && activeSession == anaSessionCountPM43.Third))
				{
					if (!existsHistDailyData)	
						currentClose	= close;
			...
			else if (selectedSession != anaSessionTypePM43.UserDefined && Bars.BarsType.IsIntraday && calcOpen)
			{	
				if (selectedSession != anaSessionTypePM43.DailyBars) 
				{
					if (!existsHistDailyData)
			...
			if(FirstTickOfBar)
			{
				if(plotPivots && !(selectedSession == anaSessionTypePM43.RTH && activeSession == anaSessionCountPM43.Third && numberOfSessions == 2))
				{
					PP.Set(pp);
					R1.Set(r1);
					S1.Set(s1);
					...
					if(PivotFormula != anaPivotStylesPM43.Camarilla)
					{
					...
					if (Instrument.MasterInstrument.InstrumentType != Cbi.InstrumentType.Index && Instrument.MasterInstrument.InstrumentType != Cbi.InstrumentType.Currency 
						&& previousVWAP > double.Epsilon && selectedSession != anaSessionTypePM43.UserDefined && Bars.BarsType.IsIntraday)
				...
				if (existsHistDailyData && selectedSession == anaSessionTypePM43.RTH 
		...
		public anaPivotStylesPM43 PivotFormula
		{
		...
		public anaSessionTypePM43 PivotSession
		{
		...
		public anaSessionCountPM43 ActiveSession
		{
		...
		public anaCalcModePM43 CalcMode
		{
			get { return calcMode; }
			set { calcMode = value; }
		}
		...
		public anaPlotAlignPM43 PlotLabels
		{
		...
		/// <summary>
		/// </summary>
		[Description("Opacity for monthly pivot range")]
		[Category("Plot Parameters")]
		...
		/// <summary>
		/// </summary>
		[Description("Opacity for monthly Jackson Zones")]
		[Category("Plot Parameters")]
		...
		/// </summary>
		[XmlIgnore]
		[Description("Select color for monthly pivot range")]
		[Category("Plot Colors")]
		...
		/// </summary>
		[XmlIgnore]
		[Description("Select color for monthly central range")]
		[Category("Plot Colors")]
		...
		/// </summary>
		[XmlIgnore]
		[Description("Select color for monthly Jackson Zones")]
		[Category("Plot Colors")]
		...
		public override void Plot(Graphics graphics, Rectangle bounds, double min, double max)
		{
			...
			if (Bars.Count > 0 && plotStart.Date > lastBarSessionBegin.Date && selectedSession != anaSessionTypePM43.UserDefined)
			...
			{	
				...
				if (PivotFormula == anaPivotStylesPM43.Jackson && opacityJZ > 0)
				{
				...
				for (int seriesCount = 0; seriesCount < Values.Length; seriesCount++)
				{
					...
					using (GraphicsPath	path = new GraphicsPath()) 
					{
						...
						if (PlotLabels == anaPlotAlignPM43.RightAll || (firstLoop && PlotLabels == anaPlotAlignPM43.Right))
						...
						if (PlotLabels == anaPlotAlignPM43.LeftAll || (firstLoop && PlotLabels == anaPlotAlignPM43.Left))
...
namespace NinjaTrader.Indicator
{
    ...
    {
        private anaPivotsMonthlyV43[] cacheanaPivotsMonthlyV43 = null;

        private static anaPivotsMonthlyV43 checkanaPivotsMonthlyV43 = new anaPivotsMonthlyV43();

        /// <summary>
        /// Monthly Session Pivots.
        /// </summary>
        /// <returns></returns>
        public anaPivotsMonthlyV43 anaPivotsMonthlyV43(anaSessionCountPM43 activeSession, anaCalcModePM43 calcMode, anaPivotStylesPM43 pivotFormula, anaSessionTypePM43 pivotSession, bool showMidpivots, bool showPriorPeriods, double userDefinedClose, double userDefinedHigh, double userDefinedLow)
        {
            return anaPivotsMonthlyV43(Input, activeSession, calcMode, pivotFormula, pivotSession, showMidpivots, showPriorPeriods, userDefinedClose, userDefinedHigh, userDefinedLow);
        }
