...
{
    /// <summary>
    /// The ZZSwingDebug indicator plots lines that represents the swing high and low points.
    /// </summary>
    [Description("The ZZSwingDebug indicator plots lines that represents the swing high and low points.")]
    [Gui.Design.DisplayName("ZZSwingDebug (High/Low)")]
    public class ZZSwingDebug : Indicator
    {
        #region Variables
		...
		private DataSeries swingSizeSeries;
		private double		swingAvg;
		private double		swingAvgShort;
        #endregion
        ...
        {
            Add(new Plot(Color.Green, PlotStyle.Dot, "ZZSwingDebug high"));
            Add(new Plot(Color.Orange, PlotStyle.Dot, "ZZSwingDebug low"));
			Plots[0].Pen.Width		= 2;
			...
			swingSizeSeries		= new DataSeries(this);
...
namespace NinjaTrader.Indicator
{
    ...
    {
        private ZZSwingDebug[] cacheZZSwingDebug = null;

        private static ZZSwingDebug checkZZSwingDebug = new ZZSwingDebug();

        /// <summary>
        /// The ZZSwingDebug indicator plots lines that represents the swing high and low points.
        /// </summary>
        /// <returns></returns>
        public ZZSwingDebug ZZSwingDebug(int strength)
        {
            return ZZSwingDebug(Input, strength);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public ZZSwingDebug ZZSwingDebug(Data.IDataSeries input, int strength)
        {
            if (cacheZZSwingDebug != null)
                for (int idx = 0; idx < cacheZZSwingDebug.Length; idx++)
                    if (cacheZZSwingDebug[idx].Strength == strength && cacheZZSwingDebug[idx].EqualsInput(input))
                        return cacheZZSwingDebug[idx];

            lock (checkZZSwingDebug)
            {
                checkZZSwingDebug.Strength = strength;
                strength = checkZZSwingDebug.Strength;
                ...
                ZZSwingDebug indicator = new ZZSwingDebug();
                ...
                ZZSwingDebug[] tmp = new ZZSwingDebug[cacheZZSwingDebug == null ? 1 : cacheZZSwingDebug.Length + 1];
                if (cacheZZSwingDebug != null)
                    cacheZZSwingDebug.CopyTo(tmp, 0);
                tmp[tmp.Length - 1] = indicator;
                cacheZZSwingDebug = tmp;
                return indicator;
            }
        }
    }
...
namespace NinjaTrader.MarketAnalyzer
{
    public partial class Column : ColumnBase
    {
        ...
        public Indicator.ZZSwingDebug ZZSwingDebug(int strength)
        {
            return _indicator.ZZSwingDebug(Input, strength);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public Indicator.ZZSwingDebug ZZSwingDebug(Data.IDataSeries input, int strength)
        {
            return _indicator.ZZSwingDebug(input, strength);
        }
    }
